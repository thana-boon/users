...
รันจบในคำสั่งเดียว ไม่ต้องยืนยันอะไร — ลำดับคือ
`migrate` (push schema, `--force`) → `seed-admin` (สร้างบัญชีแอดมินจาก `SEED_ADMIN_*`) → `app`
ทั้งสองขั้นเป็น idempotent รันซ้ำได้
...
| `SEED_ADMIN_CODE` / `SEED_ADMIN_PASSWORD` / `SEED_ADMIN_NAME` | บัญชี teacher-admin ที่ service `seed-admin` สร้างให้ตอน `up` (NAME ไม่บังคับ) |
