...
| `SESSION_PWA_ENABLED` | เปิด/ปิด session อายุยาวสำหรับ PWA ทั้งระบบ (ดีฟอลต์ `true`) — ตั้ง `false` แล้ว session แบบ pwa ที่ออกไปแล้วตายทันที |
| `SESSION_PWA_IDLE_DAYS` | หน้าต่าง idle ของ session แบบ pwa (ดีฟอลต์ `400` วัน = อายุคุกกี้ที่เบราว์เซอร์ยอมเก็บนานที่สุด, ต่ออายุให้เองทุกวันที่เปิดใช้) |
| `SESSION_PWA_ABSOLUTE_DAYS` | เพดานของ session แบบ pwa — **ไม่ตั้ง = ไม่มีเพดาน** (ดีฟอลต์). ใส่จำนวนวันถ้าอยากให้มี |
| `SESSION_PWA_ADMIN_HOURS` | เพดานเฉพาะ session แบบ pwa **ที่มีสิทธิ์ `users:write`** — ไม่ตั้ง = ใช้ค่าเดียวกับคนอื่น (ดีฟอลต์). ใส่ `24` เพื่อกลับไปตัดที่ 1 วันแบบเดิม |
...
- Login API สาธารณะสำหรับนักเรียน/ครู (`/api/auth/{student,teacher}-login`) — decrypt แล้วเทียบรหัสผ่าน,
  ...
  - **Lockout นับแยกราย IP** (`LOGIN_LOCKOUT_MAX_FAILS`, ค่าเริ่มต้น 5) — ใส่ผิดครบจำนวนจะล็อกเฉพาะ
    เครื่อง/ที่อยู่ที่ใส่ผิด ไม่ล็อกเจ้าของบัญชีที่เครื่องอื่น. ถ้าล็อกทั้งบัญชีที่ 5 ครั้ง ใครก็ตามที่รู้
    รหัสประจำตัว (ซึ่งอยู่บนบัตรนักเรียน) ก็แกล้งล็อกคนอื่นออกจากระบบได้จากมือถือตัวเอง.
  - **ล็อกทั้งบัญชี** เกิดเมื่อรวมทุก IP แล้วผิดถึง `LOGIN_LOCKOUT_GLOBAL_MAX_FAILS` (ค่าเริ่มต้น 30)
    ในหน้าต่างเดียวกัน — กันการเดารหัสแบบกระจายหลาย IP ซึ่งคนแกล้งกันเครื่องเดียวทำไม่ถึง.
- **Session timeout 2 ชั้น** (`src/lib/jwt.ts`):
  ...
  - **PWA บนมือถือเป็นข้อยกเว้น และไม่มีวันหมดอายุ** — ตอนล็อกอินส่ง `client: "pwa"` มาได้
    (หน้า login ของโมดูลนี้ส่งให้เองเมื่อถูกเปิดจากแอปที่ติดตั้งแล้ว) จะ **ไม่มีเพดาน**
    นับจาก login เลย ทุกสิทธิ์ทั้งแอดมิน ครู และนักเรียน ส่วน idle ตั้งไว้ที่ 400 วัน
    (อายุคุกกี้ที่เบราว์เซอร์ยอมเก็บนานที่สุด) แล้วเขียนคุกกี้ใหม่ให้ทุกวันที่เปิดใช้ —
    ผลคือเปิดแอปปีละครั้งก็ยังไม่หลุด เหมือนแอปอื่นบนมือถือ. ทำแบบนี้ได้เพราะเครื่องเป็นของ
    คนเดียวและมีรหัสล็อกหน้าจอของตัวเองอยู่แล้ว และเพราะคำตอบของ "มือถือหาย" คือ
    `SESSION_PWA_ENABLED=false` ที่ฆ่า session แบบ pwa ทุกอันทันที ไม่ใช่เพดานสั้น ๆ
    ที่รบกวนคนทั้งโรงเรียนทุกวัน. ถ้าอยากได้เพดานกลับมา ตั้ง `SESSION_PWA_ABSOLUTE_DAYS`
    (ทุกคน) หรือ `SESSION_PWA_ADMIN_HOURS` (เฉพาะ `users:write`).
    ค่าที่แจ้งมาเปลี่ยนได้แค่ **อายุ** ไม่ได้เปลี่ยน **สิทธิ์** และเปลี่ยนกลางทางไม่ได้ —
    `refresh` ต่ออายุให้เท่าเดิมเสมอ. ดู `SESSION_PWA_*` ในตาราง env ข้างบน.
  - **ปิดเบราว์เซอร์ = หลุดทันที** (ยกเว้น session แบบ pwa ข้างบน) ไม่ว่าจะผ่านไปกี่นาที — cookie ทั้งสามตัว**ไม่มี** `max-age`
