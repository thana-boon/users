...
| `SESSION_PWA_IDLE_DAYS` | หน้าต่าง idle ของ session แบบ pwa (ดีฟอลต์ `30` วัน) |
| `SESSION_PWA_ABSOLUTE_DAYS` | เพดานของ session แบบ pwa (ดีฟอลต์ `30` วัน) |
| `SESSION_PWA_ADMIN_HOURS` | เพดานของ session แบบ pwa **ที่มีสิทธิ์ `users:write`** (ดีฟอลต์ `24` ชม.) — มือถือแอดมินหาย = PII ทั้งโรงเรียน จึงสั้นกว่าคนอื่น |
...
- **Session timeout 2 ชั้น** (`src/lib/jwt.ts`):
  ...
  - **PWA บนมือถือเป็นข้อยกเว้น** — ตอนล็อกอินส่ง `client: "pwa"` มาได้ (หน้า login ของโมดูลนี้
    ส่งให้เองเมื่อถูกเปิดจากแอปที่ติดตั้งแล้ว) จะได้ idle 30 วัน / เพดาน 30 วัน และ cookie
    ที่มี `max-age` จริง เพราะเครื่องเป็นของคนเดียวและมีรหัสล็อกหน้าจอของตัวเองอยู่แล้ว
    บัญชีที่มี `users:write` ยังถูกตัดที่ 24 ชม. (มือถือแอดมินหาย = PII ทั้งโรงเรียน)
