...
| | `web` (ค่าเริ่มต้น) | `pwa` |
|---|---|---|
| ไม่ใช้งานนานเท่าไรถึงหลุด | 15 นาที | 30 วัน |
| เพดานนับจาก login | 8 ชั่วโมง | **30 วัน** — หรือ **24 ชั่วโมง** ถ้าบัญชีนั้นมีสิทธิ์ `users:write` |
| ปิดแอป/เบราว์เซอร์แล้วยังอยู่ไหม | ❌ หลุด (cookie ไม่มี `Max-Age`) | ✅ อยู่ต่อ (cookie มี `Max-Age` เท่าอายุโทเคน) |

> **ทำไมบัญชีแอดมินได้แค่ 1 วัน:** session ที่มี `users:write` อ่านข้อมูลนักเรียนได้ทั้งโรงเรียน โทรศัพท์ที่หายไปพร้อม session แบบนั้นคือ PII ทั้งโรงเรียนหลุดไปด้วย จึงตัดที่ 1 วันเพื่อให้เช้าวันรุ่งขึ้นเครื่องที่หายใช้เข้าระบบไม่ได้แล้ว (ปรับได้ที่ `SESSION_PWA_ADMIN_HOURS` ถ้าโรงเรียนตัดสินใจอีกแบบ)

**ข้อควรรู้**

- ฟิลด์นี้ **ผู้เรียกแจ้งเอง** เราพิสูจน์จากฝั่งเซิร์ฟเวอร์ไม่ได้ว่านั่นคือแอปที่ติดตั้งแล้วจริงหรือแค่แท็บหนึ่ง (การเดาจาก User-Agent ยิ่งแม่นน้อยกว่า) — ปลอดภัยเพราะมันซื้อได้แค่ **เวลา** ไม่ได้ซื้อ **สิทธิ์**: `permissions` ในโทเคนมาจากบัญชี ไม่ได้มาจากฟิลด์นี้ และเพดานของแอดมินก็ยังสั้นอยู่ดี
