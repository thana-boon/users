...
| 4.10b | `client: "pwa"` ตอนล็อกอิน | — (cookie) | ขอ session อายุยาวสำหรับ **PWA บนมือถือของผู้ใช้เอง** |
...
- `client` — `web` \| `pwa` session นี้ใช้หน้าต่างเวลาชุดไหน (ดูข้อ 4.10b) ถ้าระบบคุณออก session ของตัวเอง ให้ตั้งอายุตามค่านี้
...
> - **ล็อกทั้งบัญชี** — บัญชีนั้นถูกใส่รหัสผิดรวมกันจากหลาย IP จนถึงเพดาน (ค่าเริ่มต้น 30 ครั้ง/15 นาที) แปลว่ามีคนไล่เดารหัสอยู่จริง ไม่ใช่ผู้ใช้พิมพ์พลาด
>
> โควตาชั้นแรกนับ**แยกตาม IP ของผู้ใช้** โดยตั้งใจ ไม่งั้นใครก็ตามที่รู้รหัสประจำตัวของคนอื่น (ซึ่งไม่ใช่ความลับ) จะใส่รหัสผิดรัว ๆ เพื่อล็อกเจ้าตัวออกจากทุกระบบได้
>
> ⚠️ **ถ้าจะยิงสองเส้นทางนี้ ให้ยิงจากเบราว์เซอร์ของผู้ใช้** (อย่างในตัวอย่าง SSO ข้างบน) ไม่ใช่ให้เซิร์ฟเวอร์ของคุณยิงแทน — เพราะเราเห็นแค่ IP ของผู้เรียก ถ้าเป็นเซิร์ฟเวอร์คุณ ผู้ใช้ทุกคนจะถูกนับรวมเป็น "เครื่องเดียวกัน" ผู้ใช้คนหนึ่งพิมพ์รหัสผิด 5 ครั้ง แล้วคนที่เหลือที่ล็อกอินผ่านระบบคุณจะพลอยโดนไปด้วย
...
- `absoluteEndsAt` — เพดานนับจากตอนล็อกอินครั้งแรก **ไม่ขยับ** ไม่ว่าจะ refresh กี่ครั้ง พอถึงเวลานี้คือจบ (8 ชั่วโมงสำหรับ `web` / ยาวกว่านั้นสำหรับ `pwa` — ดูข้อ 4.10b)
- `client` — `web` \| `pwa` ค่าเดิมที่ตัดสินไว้ตอนล็อกอิน endpoint นี้ไม่เปลี่ยนให้
...
- cookie นี้ถูกลบพร้อม session ปกติตอน logout และหมดอายุตามกติกา idle 15 นาที / เพดาน 8 ชั่วโมงเดียวกัน (session แบบ `pwa` ใช้หน้าต่างของตัวเอง — ข้อ 4.10b)
- **`sso_session` เป็น session cookie** (ยกเว้น session แบบ `pwa` ซึ่งมี `Max-Age` เท่าอายุโทเคน) — ไม่มี `Max-Age`/`Expires` เบราว์เซอร์จึงลบทิ้งเองตอนปิดโปรแกรม **ปิดเบราว์เซอร์ = หลุดทั้งแพลตฟอร์ม** ต่อให้เพิ่งใช้งานไปเมื่อนาทีที่แล้ว (ตั้งใจ — เครื่องส่วนกลางในโรงเรียนต้องไม่มีบัญชีคนก่อนค้างไว้ให้คนถัดไป) ระบบคุณจึงต้องรับมือกับ `valid:false` ที่โผล่มาตอนเปิดเบราว์เซอร์ใหม่เสมอ อย่าแคชคำตอบข้ามการปิดเบราว์เซอร์ (เก็บใน `sessionStorage` ได้ อย่าเก็บใน `localStorage`)

---

### 4.10b PWA — session อายุยาวสำหรับแอปที่ติดตั้งบนมือถือ

หน้าต่าง 15 นาที / 8 ชั่วโมงข้างบนตั้งไว้สำหรับ**เครื่องกลางในห้องพักครู** ซึ่งมีคนใช้ต่อกันทั้งวัน แต่กับ **PWA ที่ผู้ใช้ติดตั้งบนมือถือตัวเอง** กติกาเดียวกันนี้ใช้ไม่ได้เลย — เครื่องเป็นของคนเดียว มีรหัสล็อกหน้าจอของตัวเองอยู่แล้ว และผู้ใช้เปิด-ปิดแอปวันละหลายสิบครั้ง ถ้าต้องพิมพ์รหัสผ่านใหม่ทุกครั้งที่เปิด ไม่มีใครติดตั้งแอปไว้

ตอนล็อกอินจึงส่งฟิลด์ `client` เพิ่มมาได้ (ทั้ง `teacher-login` และ `student-login`)

```js
await fetch('https://schoolos.example.ac.th/users/api/auth/teacher-login', {
  method: 'POST',
  credentials: 'include',
...
**ได้อะไรเมื่อ `client: "pwa"`**

| | `web` (ค่าเริ่มต้น) | `pwa` |
|---|---|---|
| ไม่ใช้งานนานเท่าไรถึงหลุด | 15 นาที | 400 วัน (ต่ออายุให้เองทุกวันที่เปิดใช้) |
| เพดานนับจาก login | 8 ชั่วโมง | **ไม่มีเพดาน** — ทุกสิทธิ์ รวมทั้งบัญชีที่มี `users:write` |
| ปิดแอป/เบราว์เซอร์แล้วยังอยู่ไหม | ❌ หลุด (cookie ไม่มี `Max-Age`) | ✅ อยู่ต่อ (cookie มี `Max-Age` เท่าอายุโทเคน) |

> **ทำไม PWA ไม่มีวันหมดอายุ:** แอปที่ติดตั้งบนมือถือควรทำตัวเหมือนแอปอื่นบนเครื่องเดียวกัน — ล็อกอินครั้งเดียวจบ เครื่องเป็นของคนเดียวและมีรหัสล็อกหน้าจอของตัวเองอยู่แล้ว. คำตอบของ "มือถือหาย" คือ `SESSION_PWA_ENABLED=false` ซึ่งฆ่า session แบบ `pwa` ที่ออกไปแล้ว **ทุกอัน** ภายในไม่กี่นาที ไม่ใช่เพดานสั้น ๆ ที่รบกวนคนทั้งโรงเรียนทุกวันเพื่อรอวันที่มือถือหายสักเครื่อง. ถ้าโรงเรียนตัดสินใจอีกแบบ ตั้งเพดานกลับได้ที่ `SESSION_PWA_ABSOLUTE_DAYS` (ทุกคน) หรือ `SESSION_PWA_ADMIN_HOURS` (เฉพาะบัญชีที่มี `users:write`)

**ข้อควรรู้**

- ฟิลด์นี้ **ผู้เรียกแจ้งเอง** เราพิสูจน์จากฝั่งเซิร์ฟเวอร์ไม่ได้ว่านั่นคือแอปที่ติดตั้งแล้วจริงหรือแค่แท็บหนึ่ง (การเดาจาก User-Agent ยิ่งแม่นน้อยกว่า) — ปลอดภัยเพราะมันซื้อได้แค่ **เวลา** ไม่ได้ซื้อ **สิทธิ์**: `permissions` ในโทเคนมาจากบัญชี ไม่ได้มาจากฟิลด์นี้ — สิ่งที่แจ้งผิดได้อย่างมากคือ session ของแท็บหนึ่งอยู่ยาวเท่าแอปที่ติดตั้งแล้ว
- **เปลี่ยนกลางทางไม่ได้** — `client` ถูกตัดสินตอนล็อกอินแล้วติดไปกับโทเคนเลย `POST /api/auth/refresh` ต่ออายุให้เท่าเดิมเสมอ ไม่ยกระดับ `web` ขึ้นเป็น `pwa` ให้ (ไม่งั้นคุกกี้ที่ถูกขโมยไปจะถูกยืดเป็นเดือนได้ด้วยการขอ refresh เฉย ๆ) ผู้ใช้ที่ติดตั้งแอปทีหลังต้องล็อกอินใหม่ในแอปหนึ่งครั้ง
- `absoluteEndsAt` ใน `POST /api/auth/refresh` และ `/auth/handoff/redeem` เป็น **`null`** ได้ แปลว่า session นี้ไม่มีเพดาน (ค่าเริ่มต้นของ `pwa`) — อย่าเผลออ่านเป็น 0 หรือ "หมดอายุแล้ว"
- `GET /api/auth/session`, `POST /api/auth/refresh` และ `/auth/handoff/redeem` ตอบฟิลด์ `client` (`web` \| `pwa`) กลับมาด้วย ถ้าระบบคุณออก session ของตัวเอง **ให้ตั้งอายุตามค่านี้** ไม่งั้นจะกลายเป็นมือถือยัง login อยู่ที่ Users แต่หลุดที่ระบบคุณ
- โทเคนที่ออกก่อนมีฟีเจอร์นี้ไม่มี claim `client` ระบบจะถือว่าเป็น `web` — ปลอดภัยฝั่งสั้นเสมอ
- แอดมินโรงเรียนสั่งปิดทั้งระบบได้ที่ `SESSION_PWA_ENABLED=false` (เช่นตอนมีมือถือหาย) แล้ว session แบบ `pwa` ที่ออกไปแล้ว **ตายทันที** ไม่ต้องรอหมดอายุ
