...
| `students:contact:write` | **เขียน** เบอร์/ผู้ติดต่อฉุกเฉินกลับเข้าระบบ | ⚠️ **สิทธิ์เขียนอันเดียวของ API นี้** — ดูข้อ 4.3b |
...
| **เบอร์/ผู้ติดต่อฉุกเฉิน** + เบอร์ผู้ปกครอง (ผู้ปกครอง/บิดา/มารดา) | **อ่อนไหว** | `students:contact` | ✅ `?include=contact` · audit ทุกครั้ง |
...
| 4.3b | `GET`/`PATCH /students/{id}/emergency-contact` | `students:contact` / `students:contact:write` | อ่าน/**เขียน** เบอร์ฉุกเฉินกลับเข้าระบบ |
...
| ผู้ปกครองเป็น **read-only** | ดูได้ แก้ไม่ได้ผ่าน API — เป็นข้อมูลของบุคคลที่สาม ดูข้อ 4.3b |
...
### 4.3b `GET`/`PATCH /api/public/v1/students/{id}/emergency-contact` — เบอร์ฉุกเฉิน (เขียนได้)

**นี่คือ endpoint เดียวของ API นี้ที่ "เขียน" ข้อมูลกลับเข้าฐานข้อมูลได้**

มีไว้สำหรับกรณีที่โรงเรียนเก็บเบอร์ฉุกเฉินผ่านฟอร์มของ**ระบบอื่น** แล้วส่งกลับมาให้ SchoolOS
แทนที่จะให้ธุรการพิมพ์ซ้ำ (ซึ่งคือสาเหตุที่เบอร์ที่อัปเดตเดือนมีนาคม ยังเป็นเบอร์เก่าอยู่ในเดือนกันยายน)

#### อ่าน — `GET` (scope: `students:contact`)

```
GET /api/public/v1/students/123/emergency-contact
```

ตอบ `{ "data": { ... } }` โดย `data` มีรูปร่างเดียวกับ `contact` ในข้อ 4.3a ทุกประการ
ใช้ตอนอยากโชว์ค่าปัจจุบันให้ผู้ปกครองเห็นก่อนแก้

#### เขียน — `PATCH` (scope: `students:contact:write`)

```bash
curl -X PATCH https://schoolos.example.ac.th/users/api/public/v1/students/123/emergency-contact \
  -H "X-API-Key: sk_live_..." \
  -H "Content-Type: application/json" \
  -d '{ "emergencyPhone": "0812345678" }'
```

**ช่องที่เขียนได้ — มีเท่านี้ ทั้งหมดเป็น optional**

| field | ตรงกับช่องในระบบ |
|---|---|
| `emergencyPhone` | เบอร์ติดต่อฉุกเฉิน ← **ช่องหลักที่ทุกคนใช้** |
| `emergencyEmail` | อีเมลติดต่อฉุกเฉิน |
| `homePhone` | เบอร์โทรศัพท์บ้าน (ของที่อยู่ปัจจุบัน) |
| `livingWith` / `livingWithLastname` | ปัจจุบันอาศัยอยู่กับ (ชื่อ / นามสกุล) |
| `nearbyFriendName` / `nearbyFriendLastname` / `nearbyFriendPhone` | เพื่อนใกล้บ้าน |

**Response**

```json
{
  "data": { "emergencyPhone": "0812345678", "emergencyEmail": null, "...": "..." },
  "updated": ["emergencyPhone"]
}
```

`data` คือค่า**ที่บันทึกจริง** ไม่ใช่ค่าที่ส่งมา (ดูเรื่อง normalize ด้านล่าง) · `updated` คือรายชื่อช่องที่ถูกแตะ

**กติกาที่บังคับในโค้ด**

| กฎ | ทำไม |
|---|---|
| **PATCH = ส่งเฉพาะช่องที่จะแก้** | ช่องที่ไม่ส่งมา = ไม่แตะ ส่งมาแค่ `emergencyPhone` ช่องเดียว จะไม่ไปล้างช่องอื่นทิ้ง |
| ล้างค่า = ส่ง `null` หรือ `""` | "เบอร์นี้ใช้ไม่ได้แล้ว" เป็นข้อมูลจริงที่ต้องบอกกันได้ |
| ส่ง field แปลกปลอม = **400** | ไม่ใช่เงียบ ๆ แล้วไม่ทำอะไร — ระบบที่คิดว่าแก้ชื่อได้ ต้องถูกบอกว่าแก้ไม่ได้ |
| **แตะได้เฉพาะช่องในตารางข้างบน** | สร้าง/ลบนักเรียนไม่ได้ · แก้ชื่อ/ชั้น/ห้อง/เลขบัตรไม่ได้ · แก้ที่อยู่ตามทะเบียนบ้านไม่ได้ · **แก้ข้อมูลผู้ปกครองไม่ได้** |
| เบอร์ถูก **normalize** ก่อนบันทึก | `"0812345678-"` → `"0812345678"` (ตัดอักขระคั่นหัว-ท้ายทิ้ง ไม่แตะเลขและไม่แตะขีดตรงกลาง) — ช่องที่มีแต่เครื่องหมายจะกลายเป็น `null` |
| เด็กในถังขยะ = **404** | เหมือนทุก endpoint |
| ทุกการเขียน **ถูก audit** | บันทึกว่า key ไหน แก้ของใคร ช่องอะไร เมื่อไหร่ |

> **ทำไมแก้ข้อมูลผู้ปกครองไม่ได้** — ผู้ปกครองเป็นข้อมูลของ*บุคคลที่สาม* ที่ไม่เคยมาตกลงอะไรกับระบบคุณ
> ช่อง `emergencyPhone` ครอบคลุมเคส "เบอร์เราเปลี่ยน" อยู่แล้ว โดยไม่ต้องเปิดให้ระบบนอกเขียนทับทะเบียนผู้ปกครอง
...
| 400 | `invalid_body` | body ของ `PATCH` ผิดรูป หรือมี field ที่เขียนไม่ได้ (ข้อ 4.3b) | ส่งเฉพาะช่องในตารางข้อ 4.3b |
...
| **เขียน**ข้อมูลอื่นนอกจากเบอร์ฉุกเฉิน | API เป็น read-only ทุกอย่าง ยกเว้นข้อ 4.3b | แก้ในหน้าเว็บของ SchoolOS |
