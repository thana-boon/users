...
- ขอ `?include=` block ที่ key ไม่มีสิทธิ์ → **403 `insufficient_scope`** ไม่ใช่ตอบกลับแบบไม่มี block นั้น
  (ระบบปลายทางต้องไม่มีทางเข้าใจผิดว่า "เด็กคนนี้ไม่มีประวัติแพ้ยา" ทั้งที่จริงคือ "ไม่มีสิทธิ์ถาม")

### 3.1 อันไหน "ทั่วไป" อันไหน "อ่อนไหว" — สรุปให้ครบ

ตารางนี้คือคำตอบว่า **ตอนนี้ API ส่งอะไรออกได้บ้าง** และอะไรอยู่ระดับไหน

#### นักเรียน

| ข้อมูล | ระดับ | scope ที่ต้องมี | ส่งออกได้ไหม |
|---|---|---|---|
| ชื่อ-สกุล (ไทย/อังกฤษ), ชื่อเล่น, คำนำหน้า | ทั่วไป | `students:read` | ✅ อยู่ใน list เลย |
| รหัสนักเรียน, `id` | ทั่วไป | `students:read` | ✅ |
| เพศ, วันเกิด | ทั่วไป | `students:read` | ✅ |
| ชั้น/ห้อง/เลขที่, สถานะ, ประวัติรายปี | ทั่วไป | `students:read` | ✅ |
| อีเมล, เบอร์โทรของนักเรียน | ทั่วไป | `students:read` | ✅ |
| **เบอร์เพิ่มเติม** (`additionalPhone`) | ทั่วไป | `students:read` | ✅ อยู่ใน list เลย · **ช่องเดียวที่ API เขียนกลับได้** ด้วย `students:phone:write` (ข้อ 4.3c) |
...
#### ครู / คนงาน / อาจารย์พิเศษ

| ข้อมูล | ระดับ | scope ที่ต้องมี | ส่งออกได้ไหม |
|---|---|---|---|
| ชื่อ-สกุล, รหัสครู, อีเมล | ทั่วไป | `teachers:read` | ✅ |
| **เบอร์โทร, ไอดีไลน์, วันเกิด** | ทั่วไป | `teachers:read` | ✅ — เป็นสมุดโทรศัพท์บุคลากร ตั้งใจให้อยู่ระดับนี้ |
...
| ข้อมูลสุขภาพ | — | — | ❌ ระบบไม่ได้เก็บของครู (เก็บเฉพาะนักเรียน) |

> **คนงาน** (`workers:*`) และ **อาจารย์พิเศษ** (`special-teachers:*`) ใช้กติกาเดียวกับครูทุกประการ
> ต่างที่อาจารย์พิเศษไม่มี `:pii` เพราะตารางไม่ได้เก็บเลขบัตร ปชช. ไว้เลย
...
| เรื่อง | รายละเอียด |
|---|---|
| ไม่มีข้อมูล ≠ ไม่มี key | เด็กที่ยังไม่เคยกรอกสุขภาพ จะได้ `health` ที่เป็น **null ทุกช่อง** ไม่ใช่ `health: null` และไม่ใช่หายไปเฉย ๆ — โครงสร้างเหมือนกันเสมอ |
...
### 4.3b `GET /api/public/v1/students/{id}/emergency-contact` — เบอร์ฉุกเฉิน (อ่านอย่างเดียว)

```
GET /api/public/v1/students/123/emergency-contact
```

scope: `students:contact` · ตอบ `{ "data": { ... } }` โดย `data` มีรูปร่างเดียวกับ `contact` ในข้อ 4.3a ทุกประการ
ใช้ตอนอยากได้แค่ block นี้ block เดียว ไม่ต้องดึง row นักเรียนทั้งแถว

> **⚠️ เปลี่ยนแปลง — `PATCH` ของ endpoint นี้ถูกยกเลิกแล้ว**
> เดิมระบบนอกเขียน `emergencyPhone` / `homePhone` / `livingWith` / เพื่อนใกล้บ้าน กลับมาได้ด้วย scope `students:contact:write`
> ตอนนี้ **เขียนไม่ได้แล้ว** — ยิง `PATCH` มาจะได้ `405` · scope `students:contact:write` ถูกยกเลิก key ที่ยังมีสตริงนี้ติดอยู่จะไม่ได้สิทธิ์อะไรจากมัน
> **ย้ายไปใช้ข้อ 4.3c แทน** (ช่อง "เบอร์เพิ่มเติม")
>
> **ทำไม** — เบอร์ฉุกเฉินคือเบอร์ที่โรงเรียนโทรตอนเด็กเจ็บ เป็นช่องสุดท้ายที่ควรอยู่ใต้การควบคุมของระบบอื่น
> ฟอร์มที่กรอกผิด หรือ key ที่หลุด จะทับเบอร์นั้นทิ้งโดยไม่มีใครรู้จนกว่าจะโทรไม่ติด
> ช่อง "เบอร์เพิ่มเติม" เป็นช่องที่ **ธุรการไม่ได้กรอก** ทำขึ้นมาเพื่อการนี้โดยเฉพาะ — กรณีแย่ที่สุดคือมีเบอร์เกินมาหนึ่งเบอร์ ไม่ใช่เบอร์จริงหาย
...
#### อ่าน — `GET` (scope: `students:read`)

```
GET /api/public/v1/students/123/additional-phone
```

```json
{ "data": { "id": 123, "studentCode": "10234", "additionalPhone": "0898850863" } }
```

ค่าเดียวกันนี้ติดมากับ row ในข้อ 4.2 และ 4.3 อยู่แล้ว — endpoint นี้มีไว้ให้ยิงถามทีละคนสั้น ๆ ก่อนเขียนทับ

#### เขียน — `PATCH` (scope: `students:phone:write`)

```bash
curl -X PATCH https://schoolos.example.ac.th/users/api/public/v1/students/123/additional-phone   -H "X-API-Key: sk_live_..."   -H "Content-Type: application/json"   -d '{ "additionalPhone": "081-234-5678" }'
```

**ช่องที่เขียนได้ — มีช่องเดียว**

| field | ตรงกับช่องในระบบ | จำเป็น |
|---|---|---|
| `additionalPhone` | **เบอร์เพิ่มเติม** ในหน้าประวัตินักเรียน | ✅ ต้องส่งเสมอ (ส่ง `null` = ล้างค่า) |

**Response**

```json
{
  "data": { "id": 123, "studentCode": "10234", "additionalPhone": "0812345678" },
  "changed": true,
  "warning": null
}
```
...
| ฟิลด์ | ความหมาย |
|---|---|
| `data.additionalPhone` | ค่า**ที่บันทึกจริง** ไม่ใช่ค่าที่ส่งมา — `"081-234-5678"` ถูกเก็บเป็น `"0812345678"` |
| `changed` | `false` เมื่อค่าที่ส่งมาเท่ากับค่าเดิมอยู่แล้ว (ไม่ใช่ error — sync ซ้ำได้ปลอดภัย) |
| `warning` | ข้อความเตือนเมื่อจำนวนหลักดูแปลก (ปกติเบอร์บ้าน 9 หลัก มือถือ 10) — **เตือนเฉย ๆ ค่าถูกบันทึกแล้ว** |

**กติกาที่บังคับในโค้ด**

| กฎ | ทำไม |
|---|---|
| **แตะได้เฉพาะ `additionalPhone`** | สร้าง/ลบนักเรียนไม่ได้ · แก้ชื่อ/ชั้น/ห้อง/เลขบัตร/ที่อยู่/สุขภาพไม่ได้ · **แก้เบอร์ฉุกเฉินและข้อมูลผู้ปกครองไม่ได้** |
| ส่ง field แปลกปลอม = **400 `invalid_body`** | ไม่ใช่เงียบ ๆ แล้วไม่ทำอะไร — ระบบที่คิดว่าแก้ชื่อได้ ต้องถูกบอกว่าแก้ไม่ได้ |
| ล้างค่า = ส่ง `null` หรือ `""` | "เบอร์นี้ใช้ไม่ได้แล้ว" เป็นข้อมูลจริงที่ต้องบอกกันได้ |
| เบอร์ถูก **normalize** ก่อนบันทึก | เก็บเป็น**ตัวเลขล้วน** — `"089-885-0863"` → `"0898850863"` · ค่าที่ไม่มีตัวเลขเลย (`"-"`, `"ไม่มี"`) กลายเป็น `null` |
...
| 400 | `invalid_body` | body ของ `PATCH` ผิดรูป หรือมี field ที่เขียนไม่ได้ (ข้อ 4.3c) | ส่งแค่ `additionalPhone` ช่องเดียว |
